...
def test_start_debate_without_engine(client):
    """When engine isn't initialised, get_engine returns 400."""
    # No fake_engine fixture here — TestClient created without `with` doesn't
    # run lifespan, so app.state.engine is None.
    app.dependency_overrides.pop(get_engine, None)
    resp = client.post(
        "/api/debate/start",
...
async def test_safe_judge_emits_judge_error_on_exception():
    ...
    assert "kaboom" in error_events[0].payload["message"]
...
def _patch_openai_response(content: str = "优化后的话题"):
    """Build a mock AsyncOpenAI client whose completions return ``content``."""
...
def test_refine_topic_success(client, patched_settings_with_key):
    fake_client = _patch_openai_response("AI 是否应在 K-12 教育中替代教师？")
    with patch("app.routes.topic.AsyncOpenAI", return_value=fake_client):
        resp = client.post("/api/topic/refine", json={"topic": "AI 教育"})
